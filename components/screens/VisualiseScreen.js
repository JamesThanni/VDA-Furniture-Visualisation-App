...
/* React component displaying information on the space a product takes up or 
* prompting users to add items to the visual if the have not already.
*/
function VisualAnalysis({ name, title, dimensions }) {
  ...
  return name != undefined ? (
    ...
    <View style={styles.info}>
      <AppText text="Please add products to the visual from your cart. In the meantime, you may generate a visual for an example product." />
    </View>
  )
}

export default function Visualise(props) {
  const { sceneLoaded, setSceneLoaded } = React.useContext(CartDataContext)
